...
class BME680Mock(BaseSensor):
    ...
    def __init__(self):
        super().__init__(sensor_id="bme680", name="BME680 Environmental Sensor (MOCK)")
        # Base values
    ...
    async def read(self) -> Dict[str, Any]:
        ...
        temperature = self.base_temp + random.uniform(
            -self.temp_variation, self.temp_variation
        )
        humidity = self.base_humidity + random.uniform(
            -self.humidity_variation, self.humidity_variation
        )
        pressure = self.base_pressure + random.uniform(
            -self.pressure_variation, self.pressure_variation
        )
        gas_resistance = self.base_gas_resistance + random.uniform(
            -self.gas_variation, self.gas_variation
        )
